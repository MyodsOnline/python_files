...
class TextReader(ttk.Frame):
    ...
    def create_widget_elements(self):
        ...
        file_entry.pack(side=LEFT, fill=X, expand=YES, padx=(5, 0), pady=10)

        browse_btn = ttk.Button(self, text='Browse', command=self.open_file)
        browse_btn.pack(side=LEFT, fill=X, padx=(5, 0), pady=10)

        quit_btn = ttk.Button(self, text='Quit', command=self.master.destroy)
        quit_btn.pack(side=RIGHT, fill=X, padx=(5, 0), pady=10)
